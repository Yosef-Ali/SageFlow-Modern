...
--
-- Usage: supabase.rpc('import_ptb_v2', { p_company_id: '...', p_data: { ... } })

create or replace function import_ptb_v2(p_company_id text, p_data jsonb)
returns jsonb
language plpgsql
security definer
as $$
    ...
    for v_rec in select * from jsonb_array_elements(p_data->'customers') loop
      ...
      if v_exist_id is not null then
        ...
        set name = (v_rec->>'name'),
            ...
            balance = coalesce((v_rec->>'balance')::numeric, balance),
            updated_at = now()
        where id = v_exist_id;
      else
        insert into customers (
          ...
          'CORPORATE', 'NET_30', true, coalesce((v_rec->>'balance')::numeric, 0)
        );
      end if;
    ...
    for v_rec in select * from jsonb_array_elements(p_data->'vendors') loop
      ...
      if v_exist_id is not null then
        ...
        insert into vendors (
          ...
          gen_random_uuid(), p_company_id, 
          v_rec->>'id', v_rec->>'name', v_rec->>'email', v_rec->>'phone',
          'SUPPLIER', 'NET_30', true, coalesce((v_rec->>'balance')::numeric, 0)
        );
      end if;
    ...
    for v_rec in select * from jsonb_array_elements(p_data->'accounts') loop
      ...
      if v_exist_id is not null then
        ...
        insert into chart_of_accounts (
          ...
          (v_rec->>'type')::account_type, coalesce((v_rec->>'balance')::numeric, 0), true
        );
      end if;
      v_a_cnt := v_a_cnt + 1;
    end loop;
