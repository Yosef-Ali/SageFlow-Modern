...
export function useItem(id: string) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const result = await getItem(id, companyId)
      if (!result.success) {
...
export function useInventorySummary() {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      if (!companyId) return { totalItems: 0, lowStockItems: 0, totalValue: 0 }
...
export function useUpdateItem() {
  ...
  return useMutation({
    ...
    onSuccess: (_, variables: { id: string; data: ItemFormValues }) => {
      queryClient.invalidateQueries({ queryKey: inventoryKeys.lists(companyId) })
